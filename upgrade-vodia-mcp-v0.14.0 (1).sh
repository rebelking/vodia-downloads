...
if [[ ${EUID} -ne 0 ]]; then
  echo "Run with sudo: sudo bash upgrade-vodia-mcp-v0.14.0.sh"
  exit 1
fi
...
[[ "$CURRENT_VERSION" == "0.13.2" ]] || {
  echo "This upgrade expects v0.13.2; found ${CURRENT_VERSION:-unknown}."
  exit 1
}
...
test -f release/vodia-mcp/RELEASE-NOTES-v0.14.0.md
...
if ! command -v node >/dev/null 2>&1 || ! node -e 'const [a,b]=process.versions.node.split(".").map(Number);process.exit(a>22||(a===22&&b>=13)?0:1)' 2>/dev/null; then
  echo "Installing Node.js 22 because Vodia MCP 0.14.0 requires Node.js 22.13+..."
...
AUTH_USER_COUNT="$(runuser -u vodiamcp -- env DB_PATH="$AUTH_DB_PATH" node "$APP_DIR/auth-cli.js" list-users | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>process.stdout.write(String(JSON.parse(s).length)))')"
if [[ "$AUTH_USER_COUNT" == "0" ]]; then
  ...
  runuser -u vodiamcp -- env DB_PATH="$AUTH_DB_PATH" VODIA_BOOTSTRAP_PASSWORD="$OAUTH_ADMIN_PASSWORD" node "$APP_DIR/auth-cli.js" create-admin --email "$OAUTH_ADMIN_EMAIL" --name "$OAUTH_ADMIN_NAME" >/dev/null
fi
chmod 0600 "$ENV_FILE"
