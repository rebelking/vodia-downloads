#!/usr/bin/env bash
# Vodia MCP v0.14.9.38 — hosted-first AWS onboarding and guided deployment
set -Eeuo pipefail

APP="${VODIA_MCP_APP_DIR:-/opt/vodia-mcp}"
ENV_FILE="${VODIA_MCP_ENV_FILE:-/etc/vodia-mcp.env}"
SERVICE="vodia-mcp"
SOURCE_COMMIT="38cfad967bbbd12c3de1db0bdc968383e40837b5"
RAW_BASE="https://raw.githubusercontent.com/rebelking/vodia-downloads/${SOURCE_COMMIT}"
NEW_URI="ui://vodia/msp-guided/v0.14.9.38/mcp-app.html"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
BACKUP="/var/backups/vodia-mcp-v0.14.9.38-guided-aws-$STAMP"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
case "$CURRENT" in
  0.14.9.37) ;;
  0.14.9.38) echo "v0.14.9.38 already installed; verification mode." ;;
  *) fail "expected v0.14.9.37 or v0.14.9.38; found ${CURRENT:-unknown}" ;;
esac
...
echo "[2/8] Download immutable v0.14.9.38 files"
...
curl -fsSL "$RAW_BASE/verify-vodia-guided-aws-deploy-v0.14.9.38.sh" -o "$TMP/verify.sh"
...
s,n=re.subn(r'(CONNECTOR_VERSION\s*=\s*["\'])0\.14\.9\.37(["\'])',r'\g<1>0.14.9.38\2',s,count=1)
if n != 1 and '0.14.9.38' not in s: raise SystemExit('PATCH ERROR: version anchor not found')
p.write_text(s)
PY
grep -q 'CONNECTOR_VERSION.*0.14.9.38' "$TMP/version.js" || fail "version staging failed"

if [[ "$CURRENT" != "0.14.9.38" ]]; then
  echo "[4/8] Install"
...
echo "$HEALTH"
grep -q '"version":"0.14.9.38"' <<<"$HEALTH" || fail "health does not report v0.14.9.38"
...
if [[ -n "$TOKEN" ]]; then
  ...
  grep -Fq "$NEW_URI" "$TMP/tools.out" || fail "tools/list does not advertise v0.14.9.38 UI URI"
...
echo "[8/8] Complete"
echo "PASS: Vodia MCP v0.14.9.38 installed and verified."
