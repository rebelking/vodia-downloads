...
The EC2 inventory backend tool must already be installed. The card works
whether or not the older V75 inventory table was added to the UI.
...
if len(re.findall(r'server\.registerTool\(\s*["\']aws_list_customer_ec2_instances["\']',s))!=1:
    raise SystemExit('PATCH ERROR: EC2 inventory backend tool is absent or ambiguous. Run --inspect and share the counts; no files changed.')
PY
