...
if ! (cd "$APP" && node --input-type=module -e 'await import("@aws-sdk/client-iam"); console.log("PASS: @aws-sdk/client-iam available")'); then
  fail "@aws-sdk/client-iam is not installed in $APP. No files were changed."
fi
...
if [[ "$DRY_RUN_ONLY" == "1" ]]; then
  ...
  echo "DRY RUN: @aws-sdk/client-iam is available."
  exit 0
fi
