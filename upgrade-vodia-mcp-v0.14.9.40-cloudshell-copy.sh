#!/usr/bin/env bash
# Vodia MCP v0.14.9.40 — embedded-frame CloudShell copy fix
set -Eeuo pipefail

APP="${VODIA_MCP_APP_DIR:-/opt/vodia-mcp}"
ENV_FILE="${VODIA_MCP_ENV_FILE:-/etc/vodia-mcp.env}"
SERVICE="vodia-mcp"
SOURCE_COMMIT="88cce4ca9856fe02f1e5a2bd370c4a3661ae1dc2"
RAW_BASE="https://raw.githubusercontent.com/rebelking/vodia-downloads/${SOURCE_COMMIT}"
NEW_URI="ui://vodia/msp-guided/v0.14.9.40/mcp-app.html"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
BACKUP="/var/backups/vodia-mcp-v0.14.9.40-guided-aws-$STAMP"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
case "$CURRENT" in
  0.14.9.37|0.14.9.38|0.14.9.39) ;;
  0.14.9.40) echo "v0.14.9.40 already installed; verification mode." ;;
  *) fail "expected v0.14.9.37 through v0.14.9.40; found ${CURRENT:-unknown}" ;;
esac
...
echo "[2/8] Download immutable v0.14.9.40 files"
...
curl -fsSL "$RAW_BASE/verify-vodia-guided-aws-deploy-v0.14.9.40.sh" -o "$TMP/verify.sh"
...
s,n=re.subn(r'(CONNECTOR_VERSION\s*=\s*["\'])0\.14\.9\.(?:37|38|39)(["\'])',r'\g<1>0.14.9.40\2',s,count=1)
if n != 1 and '0.14.9.40' not in s: raise SystemExit('PATCH ERROR: version anchor not found')
p.write_text(s)
PY
grep -q 'CONNECTOR_VERSION.*0.14.9.40' "$TMP/version.js" || fail "version staging failed"

if [[ "$CURRENT" != "0.14.9.40" ]]; then
  echo "[4/8] Install"
...
echo "$HEALTH"
grep -q '"version":"0.14.9.40"' <<<"$HEALTH" || fail "health does not report v0.14.9.40"
...
if [[ -n "$TOKEN" ]]; then
  ...
  grep -Fq "$NEW_URI" "$TMP/tools.out" || fail "tools/list does not advertise v0.14.9.40 UI URI"
...
echo "[8/8] Complete"
echo "PASS: Vodia MCP v0.14.9.40 installed and verified."
