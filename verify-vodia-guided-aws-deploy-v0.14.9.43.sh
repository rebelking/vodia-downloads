...
for f in "$UI" "$MODULE" "$CONNECTIONS" "$DEPLOY"; do [[ -f "$f" ]] || fail "missing file: $f"; done

python3 - "$UI" "$MODULE" "$CONNECTIONS" "$DEPLOY" "$TMP/app.js" <<'PY'
...
deploy=Path(sys.argv[4]).read_text()
out=Path(sys.argv[5])

checks={
  "v0.14.9.43 UI URI": 'ui://vodia/msp-guided/v0.14.9.43/mcp-app.html' in module,
  ...
  "embedded-frame copy fallback": 'document.execCommand("copy")' in ui and 'Clipboard API unavailable' in ui,
  ...
  "automatic connected-account continuation": 'advanceToAwsStep()' in ui and 'await loadDeploymentRegions()' in ui and 'Next: Choose AWS Region' in ui,
  ...
  "region-aware instance types": 'DescribeInstanceTypesCommand' in deploy and 'instanceTypesWarning' in deploy,
  "instance type selector": 'id="instanceType" disabled' in ui and 'Free-plan eligible' in ui,
  "guided Marketplace subscription": 'id="viewMarketplaceOffer"' in ui and 'View plans &amp; subscribe' in ui,
  "live buyer offer and quote": 'aws_marketplace_present_vodia_offer' in ui and 'aws_marketplace_prepare_vodia_purchase' in ui,
  "exact financial approval": 'aws_marketplace_accept_vodia_purchase' in ui and 'currentMarketplaceQuote.confirmation' in ui,
  "subscription-gated deployment": 'marketplaceSubscriptionActive && currentNetwork' in ui,
  "AWS agreement API backend": 'CreateAgreementRequestCommand' in deploy and 'AcceptAgreementRequestCommand' in deploy,
  "buyer-specific offer choices": 'availableOffers' in deploy and 'marketplaceOfferSelect' in ui,
  "testing storage default": 'id="storageGiB" type="number" min="8" max="16384" value="20"' in ui,
  ...
  "UI app version": 'appInfo:{name:"vodia-setup",version:"1.9.0"}' in ui,
}
...
echo
echo "RESULT: PASS — guided AWS onboarding, buyer-specific Marketplace subscription, exact financial approval, and deployment planning are present."
