...
if [[ ${EUID} -ne 0 ]]; then
  echo "Run with sudo: sudo bash upgrade-vodia-mcp-v0.14.6.sh"
  exit 1
fi
...
[[ "$CURRENT_VERSION" == "0.14.4" || "$CURRENT_VERSION" == "0.14.5" ]] || {
  echo "This upgrade expects v0.14.4 or v0.14.5; found ${CURRENT_VERSION:-unknown}."
  exit 1
}
...
test -f release/vodia-mcp/RELEASE-NOTES-v0.14.6.md
...
if ! command -v node >/dev/null 2>&1 || ! node -e 'const [a,b]=process.versions.node.split(".").map(Number);process.exit(a>22||(a===22&&b>=13)?0:1)' 2>/dev/null; then
  echo "Installing Node.js 22 because Vodia MCP 0.14.6 requires Node.js 22.13+..."
