#!/usr/bin/env bash
# Vodia MCP v0.14.9.41 — automatic AWS region continuation
set -Eeuo pipefail

APP="${VODIA_MCP_APP_DIR:-/opt/vodia-mcp}"
ENV_FILE="${VODIA_MCP_ENV_FILE:-/etc/vodia-mcp.env}"
SERVICE="vodia-mcp"
SOURCE_COMMIT="cb14770ee4ff1da240f1c8248a2139497e4961d3"
RAW_BASE="https://raw.githubusercontent.com/rebelking/vodia-downloads/${SOURCE_COMMIT}"
NEW_URI="ui://vodia/msp-guided/v0.14.9.41/mcp-app.html"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
BACKUP="/var/backups/vodia-mcp-v0.14.9.41-guided-aws-$STAMP"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
case "$CURRENT" in
  0.14.9.37|0.14.9.38|0.14.9.39|0.14.9.40) ;;
  0.14.9.41) echo "v0.14.9.41 already installed; verification mode." ;;
  *) fail "expected v0.14.9.37 through v0.14.9.41; found ${CURRENT:-unknown}" ;;
esac
...
echo "[2/8] Download immutable v0.14.9.41 files"
...
curl -fsSL "$RAW_BASE/verify-vodia-guided-aws-deploy-v0.14.9.41.sh" -o "$TMP/verify.sh"
...
s,n=re.subn(r'(CONNECTOR_VERSION\s*=\s*["\'])0\.14\.9\.(?:37|38|39|40)(["\'])',r'\g<1>0.14.9.41\2',s,count=1)
if n != 1 and '0.14.9.41' not in s: raise SystemExit('PATCH ERROR: version anchor not found')
p.write_text(s)
PY
grep -q 'CONNECTOR_VERSION.*0.14.9.41' "$TMP/version.js" || fail "version staging failed"

if [[ "$CURRENT" != "0.14.9.41" ]]; then
  echo "[4/8] Install"
...
echo "$HEALTH"
grep -q '"version":"0.14.9.41"' <<<"$HEALTH" || fail "health does not report v0.14.9.41"
...
if [[ -n "$TOKEN" ]]; then
  ...
  grep -Fq "$NEW_URI" "$TMP/tools.out" || fail "tools/list does not advertise v0.14.9.41 UI URI"
...
echo "[8/8] Complete"
echo "PASS: Vodia MCP v0.14.9.41 installed and verified."
