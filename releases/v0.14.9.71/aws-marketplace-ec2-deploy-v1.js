...
async function scanVodiaManagedInventory(roleArn, externalId, customerId, productId) {
  ...
  const checks=await Promise.allSettled(regions.map(async region=>{
    ...
    const filters=[
      {Name:"tag:ManagedBy",Values:["VodiaMCP"]},
      {Name:"instance-state-name",Values:VODIA_ACTIVE_INVENTORY_STATES}
    ];
    if(productId) filters.push({Name:"tag:VodiaMarketplaceProductId",Values:[productId]});
    const out=await client.send(new DescribeInstancesCommand({Filters:filters}));
    return (out.Reservations || []).flatMap(reservation=>reservation.Instances || []).map(instance=>({region,instance}));
  }));
  const regionErrors=[];
  const raw=[];
