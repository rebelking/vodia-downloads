...
if [[ ${EUID} -ne 0 ]]; then
  echo "Run with sudo: sudo bash upgrade-vodia-mcp-v0.14.3.sh"
  exit 1
fi
...
test -f release/vodia-mcp/RELEASE-NOTES-v0.14.3.md
...
if ! command -v node >/dev/null 2>&1 || ! node -e 'const [a,b]=process.versions.node.split(".").map(Number);process.exit(a>22||(a===22&&b>=13)?0:1)' 2>/dev/null; then
  echo "Installing Node.js 22 because Vodia MCP 0.14.3 requires Node.js 22.13+..."
