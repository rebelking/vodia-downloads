...
if [[ ${EUID} -ne 0 ]]; then
  echo "Run with sudo: sudo bash upgrade-vodia-mcp-v0.14.8.sh"
  exit 1
fi
...
[[ "$CURRENT_VERSION" == "0.14.4" || "$CURRENT_VERSION" == "0.14.5" || "$CURRENT_VERSION" == "0.14.6" || "$CURRENT_VERSION" == "0.14.7" ]] || {
  echo "This upgrade expects v0.14.4 through v0.14.7; found ${CURRENT_VERSION:-unknown}."
  exit 1
}
...
if [[ -n "${VODIA_MCP_PACKAGE_FILE:-}" ]]; then
  ...
  LOCAL_CHECKSUM_FILE="${VODIA_MCP_CHECKSUM_FILE:-${VODIA_MCP_PACKAGE_FILE}.sha256}"
  ...
  cp "$LOCAL_CHECKSUM_FILE" "vodia-mcp-v${VERSION}-complete-api.zip.sha256"
else
  wget --https-only --secure-protocol=TLSv1_2 -q "${BASE_URL}/vodia-mcp-v${VERSION}-complete-api.zip"
  wget --https-only --secure-protocol=TLSv1_2 -q "${BASE_URL}/vodia-mcp-v${VERSION}-complete-api.zip.sha256"
fi

echo "[2/8] Verifying SHA-256..."
sha256sum -c "vodia-mcp-v${VERSION}-complete-api.zip.sha256"
...
test -f release/vodia-mcp/RELEASE-NOTES-v0.14.8.md
...
if ! command -v node >/dev/null 2>&1 || ! node -e 'const [a,b]=process.versions.node.split(".").map(Number);process.exit(a>22||(a===22&&b>=13)?0:1)' 2>/dev/null; then
  echo "Installing Node.js 22 because Vodia MCP 0.14.8 requires Node.js 22.13+..."
...
for attempt in {1..30}; do
  if HEALTH_JSON="$(curl -fs http://127.0.0.1:3100/health 2>/dev/null)"; then
    break
  fi
  sleep 1
done
if [[ -z "$HEALTH_JSON" ]]; then
  echo "Health endpoint did not become ready within 30 seconds."
  journalctl -u vodia-mcp -n 40 --no-pager || true
  exit 1
fi
