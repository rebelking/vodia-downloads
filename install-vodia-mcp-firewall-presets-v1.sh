#!/usr/bin/env bash
# Adds selectable Vodia PBX firewall presets to MCP 0.14.9.83.
set -Eeuo pipefail
MODE=${1:---dry-run}
[[ $MODE == --dry-run || $MODE == --apply ]] || { echo 'Usage: sudo bash script [--dry-run|--apply]' >&2; exit 2; }
[[ $EUID -eq 0 ]] || { echo 'Run with sudo.' >&2; exit 1; }
APP=/opt/vodia-mcp
PAYLOAD_URL=https://raw.githubusercontent.com/rebelking/vodia-downloads/2577cb4e356e28f6a69122427b5dd8b77b1b66f7/patches/vodia-mcp-firewall-presets-v2.tar.gz.b64
PAYLOAD_SHA=f1ffed1de30a4dad56ac29d418676881acf89d4744dade68945f10fa5fa8fdf0
STAGE=$(mktemp -d /tmp/vodia-firewall-presets-v1.XXXXXXXX)
chmod 700 "$STAGE"
PATCH_STARTED=0
cleanup(){
  local rc=$?
...
expected={
'aws-marketplace-ec2-deploy-v1.js':'afd05063e21ea9e3d147ee032a888af19b71e2b651fc59e0c3f3633e16c8a41a',
'ui/msp-guided-app.html':'427dc1ac7fa8c7b06b31500d10013ef21b17852bb7389427517e21e561894998',
'msp-guided-app-v1.js':'ae0a81ff82846c70becab9ed72a495339546bd023b3ca4345aed91ef2c242ef9',
'version.js':'d7df4b91eb409f5b0b166b304f5c3a7b3bdeda884f1364de86201c22ea6cf2ae'}
...
assert '0.14.9.84' in (stage/'version.js').read_text()
print('PASS: live 0.14.9.83 source matches verified code-only archive')
PY
...
[[ $before == 0.14.9.83 ]] || fail "expected health 0.14.9.83; got $before"
backup_tool="$STAGE/files/tools/backup-mcp-before-firewall-presets-v1.sh"
...
archive=$(cat /opt/vodia-mcp-backups/vodia-mcp-pre-universal-firewall-v1.latest)
...
assert receipt['verified'] and receipt['version']=='0.14.9.83'
...
if systemctl restart vodia-mcp; then
  ...
  if [[ $after == 0.14.9.84 ]]; then
    PATCH_STARTED=0
    echo "PASS: Vodia PBX firewall presets installed; MCP health $after"
...
fail "patch reverted to 0.14.9.83; verified full backup: $archive"
