...
## 19. Release Notes by Version

### Release-note accuracy policy

The v0.5 through v0.7 summaries below are reconstructed from observed deployments and retained operational history. They describe the verified release baseline but should not replace Git tags or original package manifests when those become available. Starting with v0.8, every release ZIP must contain its own immutable release notes and checksums.

### v0.5.0 — Initial Remote Read-Only MCP Baseline

**Release status:** Historical  
**Release date:** Not recorded in this runbook  
**Primary objective:** Provide safe remote PBX visibility without PBX modification capability.

#### Highlights

- Introduced the Node.js Vodia PBX MCP service.
- Added a remote MCP endpoint behind Caddy HTTPS.
- Added bearer-token authentication for MCP clients.
- Established strict read-only operation.
- Added initial system, tenant, extension, registration, trunk, dial-plan, queue, CDR, and connector-information tools.
- Added systemd service management and local health checking.
- Added an interactive installer for Ubuntu/EC2 deployments.

#### Security posture

- No destructive MCP tools.
- No general PBX write tools.
- Dedicated PBX API credentials recommended.
- Public access expected only through HTTPS.

#### Operational impact

- Required DNS pointing at the MCP host.
- Required AWS Security Group ports 80 and 443.
- Required a Vodia API username and password.
- Required the bearer token to be configured on each Codex client.

#### Known limitations

- Read-only capability only.
- Limited installer validation and upgrade handling.
- Credentials could be exposed through terminal output or manual copying.
- No controlled administrator endpoint.

### v0.6.0 — Expanded Read Catalog and Read-Only Hardening

**Release status:** Historical  
**Release date:** Not recorded in this runbook  
**Primary objective:** Expand diagnostic coverage while preserving strict read-only behavior.

#### Highlights

- Expanded the read catalog and callable GET operations.
- Added broader PBX inventory, live-state, queue, registration, CDR, statistics, and diagnostic coverage.
- Improved tenant-aware access controls.
- Improved redaction of secrets and SIP authorization material.
- Added or strengthened persistent audit logging.
- Added catalog searching and blocking of GET endpoints that behaved like actions.
- Added self-tests for read-only enforcement, tenant boundaries, redaction, and expanded tool presence.

#### Security posture

- All exposed tools remained read-only.
- Action-like GET endpoints were blocked.
- Outside-tenant and system-wide reads were blocked when tenant scope was configured.
- Secrets were redacted from responses and persistent audit output.

#### Operational impact

- Existing read clients remained conceptually compatible.
- Administrators needed to retest tool discovery because the read catalog expanded.
- Tenant-scope configuration became more important as coverage increased.

#### Known limitations

- No general administrator write workflow.
- No plan/review/apply lifecycle for PBX changes.
- Version reporting and upgrade consistency were not yet formal release gates.

### v0.7.0 — Role-Aware Administrator and Diagnostic Planning Baseline

**Release status:** Historical/observed baseline  
**Release date:** Not recorded in this runbook  
**Primary objective:** Separate read-only access from controlled administrator access.

#### Highlights

- Added a separate administrator MCP endpoint.
- Added a distinct administrator MCP bearer token.
- Added role-aware read access across authorized tenant domains.
- Added administrator-oriented inventory, status, audit, SIP trace, and troubleshooting tools.
- Added PCAP planning for extensions and SIP trunks.
- Added the controlled `apply_vodia_change` execution pattern used by PCAP changes.
- Added redacted plan, write, verification, and audit events.

#### Security posture

- Read and administrator credentials were separated.
- Administrator changes required a plan before apply.
- Exact confirmation and audit controls began forming the controlled-write model.
- Redaction remained mandatory for sensitive PBX and SIP data.

#### Operational impact

- Clients needed two MCP server definitions and two environment variables.
- Administrators needed to distinguish the dashboard token from the administrator MCP bearer token.
- Tool discovery could return different catalogs for `/mcp` and `/mcp-admin`.

#### Known limitations

- Administrator capability was focused on diagnostics and a narrow set of controlled changes.
- No broad typed account, trunk, dial-plan, or tenant-settings administration.
- Historical deployments may report `0.7.0` from connector metadata even after upgrading application files to v0.8.

### v0.8.0 — Full Typed Administrator Planning

**Release status:** Current operational baseline  
**Release date:** August 2026 deployment baseline  
**Primary objective:** Add controlled, typed administration for common PBX objects while keeping risky operations policy-gated.
...
- `apply_vodia_change`
- `full` administrator profile
- Destructive-operation feature flag
- Critical-write feature flag
- Exact confirmation enforcement
- Single-use plan enforcement
- Plan expiration and stale-state protection
- Request hashing and reason capture
- Post-apply readback verification where supported
- Expanded administrator self-tests

#### Changed

- Administrator service description changed to role-aware/controlled administrator operation.
- Existing SIP trunks can be updated, renamed, enabled, disabled, or policy-permittingly deleted through typed planners.
- Accounts can be created and updated through typed planners.
- Dial plans and tenant settings can be updated through typed planners.
- Write operations are separated from planning and require explicit apply.

#### Security

- Destructive operations remain disabled by default.
- Critical break-glass operations remain disabled by default.
- Read and administrator tokens remain separate.
- Plans must contain an actor, tenant, reason, request hash, expiration, and required confirmation.
- Applied plans are single-use.
- Sensitive values must be redacted from plans, logs, verification, and audit output.

#### Verified deployment results

- Package version reported by `package.json`: `0.8.0`.
- Service launched `/usr/bin/node /opt/vodia-mcp/http.js`.
- Administrator profile reported in configuration: `full`.
- Read MCP authentication succeeded.
- Administrator MCP authentication succeeded after the correct token was installed.
- Exact v0.8 administrator definitions were visible in Codex.
- Plan creation succeeded without applying a PBX change.
- Historical service audit showed successful PCAP plan/apply/verify operations.

#### Known issues

- `get_connector_info` can incorrectly report `0.7.0`.
- No `plan_create_sip_trunk`; new SIP-trunk creation is unsupported.
- A create plan can be generated without first proving the tenant exists and the account is unused.
- Placeholder tenant names are not rejected.
- Broad deferred discovery may omit administrator planners until queried by exact name.
- Installer output exposes credentials unless handled carefully.
- The existence check and plan can target different tenants when a prompt contains inconsistent values.

#### Upgrade notes

- Back up `/opt/vodia-mcp` and `/etc/vodia-mcp.env` before upgrading.
- Preserve tokens only when required for client continuity; otherwise rotate them.
- Restart both the service and Codex clients after changing authentication or tool catalogs.
- Re-run read, administrator, exact-tool, plan-only, and controlled-apply acceptance gates.
- Never assume package version alone proves that the running service or metadata is current.

### v0.9.0 — Planned Installation, Validation, and Full Trunk-Creation Release

**Release status:** Planned—not released  
**Target objective:** Make installation and upgrades transactional, eliminate known v0.8 validation gaps, and add controlled SIP-trunk creation.

#### Planned additions

- `plan_create_sip_trunk`
- Preflight-only validation that does not create pending plan state
- Tenant-existence validation for every planner
- Create collision detection
- Update target-existence validation
- Placeholder-value rejection
- Administrator capability/profile reporting tool
- Machine-readable redacted installation report
- Explicit installer modes: install, upgrade, repair, validate, rotate-tokens, rollback, and uninstall
- Automatic rollback following failed health or acceptance tests
- Generated tool reference from live schemas

#### Planned fixes

- Make all version surfaces report `0.9.0` from one source of truth.
- Stop printing bearer tokens during normal installation.
- Make deferred discovery reliably expose every enabled planner.
- Ensure a preflight tenant and planned tenant cannot differ.
- Validate PBX object state before issuing a change ID.
- Improve service startup diagnostics and upgrade error reporting.

#### Compatibility goals

- Preserve v0.8 read-only tool behavior.
- Preserve v0.8 plan/review/apply semantics.
- Preserve existing environment variables during normal upgrade.
- Keep destructive and critical operations disabled by default.
- Support existing Windows and macOS Codex configurations without requiring literal tokens in TOML.

#### Release blockers

- Any version mismatch between package, health, connector metadata, dashboard, and logs
- Any credential printed by default
- Missing SIP-trunk create/verify coverage
- Ability to plan against a nonexistent or placeholder tenant
- Ability to create a duplicate account or trunk without an explicit collision override policy
- Failure to reject expired, stale, replayed, or incorrectly confirmed plans
- Failed upgrade without automatic restoration of the last healthy version

### Required release-note template for v0.9 and later

Every future `RELEASE-NOTES-vX.Y.Z.md` must contain:

```markdown
# Vodia PBX MCP vX.Y.Z Release Notes
...
Release notes must never contain passwords, bearer tokens, API secrets, private keys, session IDs, or unredacted PBX authentication data.
