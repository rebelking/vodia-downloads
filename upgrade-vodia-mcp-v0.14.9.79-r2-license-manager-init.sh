...
LM_SDK_VERSION="$(cd "$APP" && node -p "require('./node_modules/@aws-sdk/client-license-manager/package.json').version" 2>/dev/null || true)"
[[ -n "$LM_SDK_VERSION" ]] || fail "could not determine installed @aws-sdk/client-license-manager version"
if (cd "$APP" && node --input-type=module -e 'await import("@aws-sdk/client-iam")' >/dev/null 2>&1); then
  ...
  echo "INFO: live install will add @aws-sdk/client-iam@$LM_SDK_VERSION to match @aws-sdk/client-license-manager"
  if npm view "@aws-sdk/client-iam@$LM_SDK_VERSION" version >/dev/null 2>&1; then
    echo "PASS: matching @aws-sdk/client-iam@$LM_SDK_VERSION is available from npm"
  else
    fail "matching @aws-sdk/client-iam@$LM_SDK_VERSION could not be resolved from npm. No files were changed."
  fi
fi
...
if [[ "$DRY_RUN_ONLY" == "1" ]]; then
  ...
  if [[ "$IAM_SDK_PRESENT" == "1" ]]; then
    ...
    echo "DRY RUN: live install will add matching @aws-sdk/client-iam@$LM_SDK_VERSION; no dependency was installed during dry run."
  fi
  exit 0
fi
...
if [[ "$IAM_SDK_PRESENT" != "1" ]]; then
  echo "Installing @aws-sdk/client-iam@$LM_SDK_VERSION"
  (cd "$APP" && npm install --save-exact "@aws-sdk/client-iam@$LM_SDK_VERSION")
fi
