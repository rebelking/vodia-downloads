#!/usr/bin/env bash
# Vodia MCP v0.14.9.71 prerequisite repair for partial v0.14.9.72
set -Eeuo pipefail
...
case "$CURRENT" in
  0.14.9.70) ;;
  0.14.9.71) echo "v0.14.9.71 detected; verifying exact-AMI markers." ;;
  0.14.9.72) echo "Partial v0.14.9.72 detected; repairing v0.14.9.71 prerequisite and temporarily rewinding the version marker to .71." ;;
  *) fail "expected v0.14.9.70, .71, or partial .72; found ${CURRENT:-unknown}." ;;
esac

NEED_PATCH=0
if ! grep -Fq 'VODIA_EXACT_MARKETPLACE_AMI_V71' "$BACKEND" || ! grep -Fq 'data-exact-marketplace-ami="v0.14.9.71"' "$UI"; then
  NEED_PATCH=1
fi

echo "=== Repair exact Marketplace AMI prerequisite (v${TO_VER}) ==="
mkdir -p "$TMP/staged"
...
rollback(){
  echo "ROLLBACK: restoring pre-repair files"
...
systemctl restart "$SERVICE"
  ...
  echo "[5/8]-[6/8] v0.14.9.71 prerequisite already healthy"
fi
...
echo "[8/8] Complete"
echo "PASS: v0.14.9.71 exact-AMI prerequisite repaired. Now rerun the v0.14.9.72 installer."
